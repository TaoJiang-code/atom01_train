...
def qingyun_hip_yaw_joint_sign_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Penalize rw hip yaw below zero and lw hip yaw above zero."""
    asset = env.scene[asset_cfg.name]
    right_hip_yaw_joint_index = asset.joint_names.index("rw_hip_yaw_joint")
    left_hip_yaw_joint_index = asset.joint_names.index("lw_hip_yaw_joint")
    left_hip_yaw_penalty = torch.where(
        asset.data.joint_pos[:, left_hip_yaw_joint_index] > 0.0,
        asset.data.joint_pos[:, left_hip_yaw_joint_index],
        0.0,
    )
    right_hip_yaw_penalty = torch.where(
        asset.data.joint_pos[:, right_hip_yaw_joint_index] < 0.0,
        -asset.data.joint_pos[:, right_hip_yaw_joint_index],
        0.0,
    )
    return left_hip_yaw_penalty + right_hip_yaw_penalty
