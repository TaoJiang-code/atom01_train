...
#
# MuJoCo sim2sim for QingYun Rev3 parkour policies exported as separate ONNX graphs
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="QingYun Rev3 parkour sim2sim (depth_encoder.onnx + actor.onnx).")
    default_export = "logs/rsl_rl/qingyun_rev3_parkour/2026-06-14_20-02-15/exported"
    parser.add_argument(
        "--export_dir",
        type=str,
        default=None,
        help=(
            "Directory containing 0-depth_encoder.onnx and actor.onnx. "
            "If set, it is used as the base path unless --depth_encoder or --actor override individual files."
        ),
    )
    parser.add_argument(
        "--depth_encoder",
        type=str,
        default=None,
        help="Path to depth encoder ONNX. Defaults to <export_dir>/0-depth_encoder.onnx.",
    )
    parser.add_argument(
        "--actor",
        type=str,
        default=None,
        help="Path to actor ONNX. Defaults to <export_dir>/actor.onnx.",
    )
    parser.add_argument(
        "--mujoco_xml",
        type=str,
    ...
    export_dir = args.export_dir or default_export
    depth_encoder_path = args.depth_encoder or os.path.join(export_dir, "0-depth_encoder.onnx")
    actor_path = args.actor or os.path.join(export_dir, "actor.onnx")
